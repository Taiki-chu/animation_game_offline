...
//	BattleGui battleGui;
	...
	/**bodyの初期座標ストック*/
	final int body_x_L_N=50,body_y_L_N=350;
...
//	}
	...
	void setbody(Graphics g) {
		//体描画
			if(getdead()) {
				BattleGui.heart_color=Color.blue;
				if(BattleGui.pertime%2==0)g.setColor(Color.black);
	...
	void setdead() {
		BattleGui.countdownR=8;
	}
	
	boolean getdead() {//障害物に接触した時の時間をカウント、カウントが終わったら終了
		BattleGui.countdownR--;
		if(BattleGui.countdownR<=0) return false;
		else return true;
	}
	public void jump_short() {//持続ジャンプができない
		
		countF = BattleGui.countF;
		JP = BattleGui.JP;
		count = BattleGui.count;
		actFlag2 = BattleGui.actFlag2;
		...
		BattleGui.countF = countF ;
		BattleGui.JP = JP ;
		BattleGui.count = count ;
		BattleGui.actFlag2 = actFlag2;
	}
